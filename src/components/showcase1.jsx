...
const Showcase1 = () => {
  ...
  // Detect if device is iPad Pro (portrait 1024x1366)
  const detectIpadPro = () => {
    const { innerWidth, innerHeight } = window;
    // Optionally, allow a tolerance range because sometimes browsers report slightly different values.
    if (
      (Math.abs(innerWidth - 1024) < 5 && Math.abs(innerHeight - 1366) < 5) ||
      (Math.abs(innerWidth - 1366) < 5 && Math.abs(innerHeight - 1024) < 5) // in case of rotation
    ) {
      setIsIpadPro(true);
    } else {
      setIsIpadPro(false);
  ...
  useEffect(() => {
    ...
    const updateContainerOffset = () => {
      if (cardContainerRef.current && carouselRef.current) {
          ...
          <div className='relative' ref={carouselRef}>
            ...
            >
              {visibleCards.map((card, index) => (
                <div
                  key={`${card.id}-${index}`}
                  className={`flex-shrink-0 p-4 rounded-2xl transition duration-300 ${
                    darkMode
                      ? 'border border-[var(--dark-hover-bg)] shadow-md'
                      : 'border border-[#e6e8f0] shadow-md'
                  }`}
                  // Use inline style override if on iPad Pro
                  style={{
                    width: isIpadPro ? '270px' : undefined,
