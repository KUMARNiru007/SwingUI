...
const GalleryCard = ({ icon, title, desc ,to }) => {
  ...
  return (
    ...
    >
      <div
        className="text-5xl mb-4 swing-ocean-gradient-text bg-[#1c1c1c] p-4 rounded-full border border-[#2ca7cf] transform transition-transform duration-300 group-hover:scale-125"
      >
        {icon}
      </div>
      <h3 className="text-lg font-bold underline">{title}</h3>
      <p className={`text-sm mt-2 ${darkMode ? 'text-white' : 'text-black'}`}>{desc}</p>
    </div>
  );
};
      ...
      >
        {duplicatedItems.map((item, i) => (
          <div key={`${direction}-${i}`} className="w-[calc(50%-0.5rem)] sm:w-auto">
            ...
            <GalleryCard icon={item.icon} title={item.title} desc={item.desc} to={item.to} />
                </Link>
          </div>
        ))}
      </div>
